...
const watchlistInstrumentIds = computed(() => [
  ...new Set(
    watchlists.value.flatMap((watchlist) => watchlist.items.map((item) => item.instrumentId)),
  ),
])
      ...
      <div v-if="activeWatchlist.items.length" class="min-h-0 flex-1 overflow-y-auto">
        ...
        >
          ...
          <div class="mr-1 text-right">
            <p
              class="font-mono text-sm leading-4"
              :class="quotes.get(item.instrumentId) ? 'text-slate-700' : 'text-slate-400'"
            >
              {{ quotes.get(item.instrumentId)?.price ?? '—' }}
            </p>
            <p
              class="mt-1 text-[10px] leading-3"
              :class="quotes.get(item.instrumentId) ? 'text-emerald-600' : 'text-slate-300'"
            >
