...
watch(watchlists, (items) => {
  if (!items.some((watchlist) => watchlist.id === activeWatchlistId.value)) activeWatchlistId.value = items[0]?.id
}, { immediate: true })

const refreshWatchlists = () => queryClient.invalidateQueries({ queryKey: ['watchlists'] })

const createWatchlistMutation = useMutation({ mutationFn: createWatchlist, onSuccess: async (watchlist) => {
  activeWatchlistId.value = watchlist.id
  await refreshWatchlists()
} })
const updateWatchlistMutation = useMutation({ mutationFn: ({ id, name }: { id: number, name: string }) => updateWatchlist(id, { name }), onSuccess: refreshWatchlists })
const deleteWatchlistMutation = useMutation({ mutationFn: deleteWatchlist, onSuccess: refreshWatchlists })
const createItemMutation = useMutation({ mutationFn: ({ watchlistId, instrumentId }: { watchlistId: number, instrumentId: number }) => createWatchlistItem(watchlistId, { instrumentId }), onSuccess: refreshWatchlists })
const updateItemMutation = useMutation({ mutationFn: ({ watchlistId, itemId, instrumentId }: { watchlistId: number, itemId: number, instrumentId: number }) => updateWatchlistItem(watchlistId, itemId, { instrumentId }), onSuccess: refreshWatchlists })
const deleteItemMutation = useMutation({ mutationFn: ({ watchlistId, itemId }: { watchlistId: number, itemId: number }) => deleteWatchlistItem(watchlistId, itemId), onSuccess: refreshWatchlists })
...
const createList = async () => {
  const { value } = await ElMessageBox.prompt('请输入自选列表名称', '新建自选列表', { inputPattern: /\S+/, inputErrorMessage: '名称不能为空' })
  await createWatchlistMutation.mutateAsync({ name: value })
}

const renameList = async () => {
  if (!activeWatchlist.value) return
  const { value } = await ElMessageBox.prompt('请输入自选列表名称', '编辑自选列表', { inputValue: activeWatchlist.value.name, inputPattern: /\S+/, inputErrorMessage: '名称不能为空' })
  await updateWatchlistMutation.mutateAsync({ id: activeWatchlist.value.id, name: value })
}

const removeList = async () => {
  if (!activeWatchlist.value) return
  await ElMessageBox.confirm(`删除“${activeWatchlist.value.name}”及其所有标的？`, '删除自选列表', { type: 'warning' })
...
const saveInstrument = async () => {
  ...
  if (editingItem.value) {
    await updateItemMutation.mutateAsync({ watchlistId: activeWatchlist.value.id, itemId: editingItem.value.id, instrumentId: selectedInstrument.value.id })
  } else {
...
const selectInstrument = (item: WatchlistItem) => {
  ...
  if (instrument) emit('selectSymbol', { id: instrument.id, symbol: instrument.symbol })
}
</script>
...
<template>
  <div class="flex h-full flex-col bg-white text-slate-800">
    ...
    <div class="scrollbar-none flex gap-1 overflow-x-auto border-b border-slate-100 px-2 pb-2">
      <button v-for="watchlist in watchlists" :key="watchlist.id" type="button" class="shrink-0 rounded-full px-3 py-1 text-xs transition-colors" :class="watchlist.id === activeWatchlistId ? 'bg-blue-500 text-white shadow-sm' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'" @click="activeWatchlistId = watchlist.id">{{ watchlist.name }}</button>
      <button type="button" class="flex size-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200" title="新建自选列表" @click="createList"><Plus :size="14" /></button>
    </div>
