...
const formatTime = (value: string | null) => value ? dayjs(value).tz('Asia/Shanghai').format('YYYY-MM-DD HH:mm') : '--'
...
<template>
  <div class="flex h-full flex-col bg-white text-slate-800">
    ...
    <div v-else class="min-h-0 flex-1 overflow-y-auto">
      <div v-for="record in records" :key="record.id" type="button" class="w-full border-b border-slate-100 px-3 py-2 text-left transition-colors hover:bg-blue-50/60" @click="emit('selectTrade', record)">
        <div class="flex items-center justify-between gap-2"><span class="truncate text-xs font-semibold">{{ record.underlyingCode }}</span><span class="text-[10px]" :class="record.direction === 'LONG' ? 'text-rose-500' : 'text-emerald-500'">{{ record.direction === 'LONG' ? '多' : '空' }} · {{ record.quantity }}手</span></div>
        ...
        <div class="mt-1 text-[10px]" :class="Number(record.realizedPnl ?? 0) >= 0 ? 'text-emerald-500' : 'text-rose-500'">盈亏 {{ formatNumber(record.realizedPnl) }}</div>
      </div>
    </div>
  </div>
</template>
