  ...
  <aside class="flex" aria-label="图表工具栏">
    <section v-if="activeTool" class="w-[280px] border-r border-slate-200 bg-white shadow-md">
      <div class="h-[calc(100%-2.5rem)]">
        <WatchlistPanel
          v-if="activeTool === 'watchlist'"
          @select-symbol="emit('selectSymbol', $event)"
        />
        <TradeRecordsPanel
          v-else-if="activeTool === 'trades'"
          @select-trade="emit('selectTrade', $event)"
        />
      </div>
    </section>
    <nav
      class="flex w-11 flex-col items-center border-r border-slate-200 bg-white/95 py-2 shadow-sm backdrop-blur"
      aria-label="图表工具"
    >
      <button
        v-for="tool in tools"
