...
import type { IPrimitivePaneRenderer, IPrimitivePaneView, ISeriesPrimitive, SeriesAttachedParameter, SeriesType, Time } from 'lightweight-charts'
...
const tableOrigin = (position: string | undefined, paneWidth: number, paneHeight: number, tableWidth: number, tableHeight: number) => {
...
class PineTableRenderer implements IPrimitivePaneRenderer {
  ...
  draw(target: CanvasRenderingTarget2D) {
    target.useBitmapCoordinateSpace(({ bitmapSize, context, horizontalPixelRatio, verticalPixelRatio }) => {
      context.save()
      for (const table of this.tables) {
        ...
        const columnWidths = Array.from({ length: columns }, (_, column) => Math.max(...table.cells.map((row) => {
          ...
          const defaultWidth = context.measureText(cell.text ?? '').width / horizontalPixelRatio + 20
          return cell.width && cell.width > 0 ? cell.width : Math.max(defaultWidth, 56)
        }), 56))
        const rowHeights = Array.from({ length: rows }, (_, row) => Math.max(...(table.cells[row] ?? []).map((cell) => (
          cell.height && cell.height > 0 ? cell.height : Math.max(fontSizeByTextSize[cell.text_size ?? 'normal'] * 1.5, 28)
        )), 28))
        const tableWidth = columnWidths.reduce((total, width) => total + width, 0)
        const tableHeight = rowHeights.reduce((total, height) => total + height, 0)
        const origin = tableOrigin(table.position, bitmapSize.width / horizontalPixelRatio, bitmapSize.height / verticalPixelRatio, tableWidth, tableHeight)
        ...
        for (let row = 0; row < rows; row += 1) {
          ...
          for (let column = 0; column < columns; column += 1) {
            ...
            context.textBaseline = cell.text_valign === 'top' ? 'top' : cell.text_valign === 'bottom' ? 'bottom' : 'middle'
            const textX = cell.text_halign === 'left' ? left + 8 * horizontalPixelRatio : cell.text_halign === 'right' ? left + scaledWidth - 8 * horizontalPixelRatio : left + scaledWidth / 2
            const textY = cell.text_valign === 'top' ? top + 5 * verticalPixelRatio : cell.text_valign === 'bottom' ? top + scaledHeight - 5 * verticalPixelRatio : top + scaledHeight / 2
        ...
        if ((table.frame_width ?? 0) > 0) {
          ...
          context.strokeRect(origin.x * horizontalPixelRatio, origin.y * verticalPixelRatio, tableWidth * horizontalPixelRatio, tableHeight * verticalPixelRatio)
        }
      }
      context.restore()
    })
