...
export class RectangleDrawingStrategy extends DrawingStrategy {
  ...
  draw(
    ...
    context.save()
    context.fillStyle =
      (drawing as DrawingCoordinates & { style?: { fillColor?: string } }).style?.fillColor ??
      'rgba(37, 99, 235, 0.10)'
  ...
  updateForDrag(context: DrawingDragContext) {
    ...
    const setLeftTime = () => {
      if (startIsLeft) start.time = current.time
      else end.time = current.time
    }
    const setRightTime = () => {
      if (startIsLeft) end.time = current.time
      else start.time = current.time
    }
    const setTopPrice = () => {
      if (startIsTop) start.price = current.price
      else end.price = current.price
    }
    const setBottomPrice = () => {
      if (startIsTop) end.price = current.price
      else start.price = current.price
    }
