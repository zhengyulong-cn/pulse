...
export function applyMomentumIndicator<T extends SeriesType>(
  ...
  class MomemtumPrimitive implements ISeriesPrimitive {
    ...
    public applyOptions(options: Partial<MomentumCalculationOptions>): void {
      this._options = {
        ...(this._options || {}),
    ...
    private _updateData = () => {
      ...
      const indicatorValues = calculateMomentumIndicatorValues(seriesData, this._options || options)
