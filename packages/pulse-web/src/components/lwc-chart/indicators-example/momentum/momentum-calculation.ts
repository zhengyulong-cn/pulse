import type { CandlestickData, LineData, WhitespaceData, UTCTimestamp } from 'lightweight-charts'
...
export function calculateMomentumIndicatorValues<TData extends SupportedData>(
  ...
  if (!source) {
    throw new Error('Please provide a `source` property for the momentum indicator.')
  }

  ensureTimestampData(data)

  const values: (number | undefined)[] = data.map((d) => (d as any)[source] as number | undefined)
  const times: UTCTimestamp[] = data.map((d) => d.time as UTCTimestamp)

  // Compute momentum
  const momentum: (number | undefined)[] = calculateMomentum(values, options.length)

  // Map to output
  return times.map((time, i): LineData<UTCTimestamp> | WhitespaceData<UTCTimestamp> =>
    typeof momentum[i] === 'number' ? { time, value: momentum[i] as number } : { time },
  )
}

function calculateMomentum(values: (number | undefined)[], length: number): (number | undefined)[] {
