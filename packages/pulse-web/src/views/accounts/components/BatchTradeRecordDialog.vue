...
const invalidRowNumbers = computed(() =>
  previewRecords.value.flatMap((record, index) =>
    requiredFields.some(
...
const canSubmit = computed(
  () =>
    previewRecords.value.length > 0 && !parseError.value && invalidRowNumbers.value.length === 0,
)
...
<template>
  ...
  >
    ...
    >
      <Landmark :size="17" /><span>记录将添加到：</span
    ...
    <div class="grid gap-4 lg:grid-cols-[minmax(320px,0.8fr)_minmax(0,1.2fr)]">
      <section class="min-w-0">
        <div class="mb-2 flex items-center justify-between gap-3">
          <h3 class="font-medium text-slate-800">粘贴 JSON</h3>
          <el-button size="small" @click="copyImportPrompt"
            ><Copy :size="14" />一键复制 prompt</el-button
          >
        </div>
        <el-input
          v-model="rawJson"
        ...
        <p v-else-if="invalidRowNumbers.length" class="mt-2 text-sm text-rose-600">
          第 {{ invalidRowNumbers.join('、') }} 条缺少必填字段。
        </p>
        <p v-else class="mt-2 text-xs leading-5 text-slate-400">
          必填：标的名称、代码、方向、手数、开仓时间、开仓价格。手续费缺失时按 0 保存。
        </p>
      </section>

      <section class="min-w-0">
        <div class="mb-2 flex items-center justify-between">
          <h3 class="font-medium text-slate-800">记录预览</h3>
          <span class="text-xs text-slate-400">{{ previewRecords.length }} 条</span>
        </div>
    ...
    <template #footer
      ...
      ><el-button type="primary" :disabled="!canSubmit" :loading="saving" @click="save"
        >新增 {{ previewRecords.length }} 条记录</el-button
      ></template
    >
  </el-dialog>
</template>
