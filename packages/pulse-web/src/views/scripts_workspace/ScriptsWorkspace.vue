...
const loadScripts = async (preferredScriptId?: number) => {
  ...
  try {
    ...
    const script = scripts.value.find((item) => item.id === preferredScriptId)
...
const saveScript = async () => {
  ...
  isSaving.value = true
  try {
    const script = await updatePineScript(selectedScript.value.id, { ...draft.value, description: draft.value.description.trim() })
...
const removeScript = async (script: PineScript) => {
  try {
    await ElMessageBox.confirm(`确定删除“${script.description}”吗？`, '删除脚本', { type: 'warning' })
  ...
  <main class="min-h-[calc(100vh-2.5rem)] bg-slate-100 px-3 py-5 text-slate-800 sm:px-5 lg:px-6">
    <div class="mx-auto max-w-[1800px]">
      <header class="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <el-button type="primary" class="!h-9 !rounded-lg !font-semibold" @click="addScript">新建脚本</el-button>
      </header>

      <div class="flex min-h-[calc(100vh-11rem)] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <aside class="flex w-80 shrink-0 flex-col bg-slate-50/60 border-r border-slate-200">
          <div class="space-y-3 border-b border-slate-200 bg-white p-4">
            <div class="flex items-center justify-between">
              ...
              <span class="rounded-md bg-slate-100 px-2 py-0.5 text-xs text-slate-400">{{ scripts.length }}</span>
            </div>
        ...
        <div v-loading="isLoading" class="min-h-0 flex-1 overflow-y-auto p-2.5">
          <button
            ...
            :class="script.id === selectedScriptId ? 'border-blue-100 bg-blue-50 text-blue-700 shadow-sm' : 'text-slate-700 hover:border-slate-200 hover:bg-white'"
          ...
          >
            ...
            <span class="shrink-0 text-slate-400 hover:text-red-500" title="删除脚本" @click.stop="removeScript(script)">×</span>
          </button>
          <p v-if="!isLoading && scripts.length === 0" class="p-6 text-center text-sm text-slate-400">暂无脚本</p>
        </div>
      </aside>
      ...
      <section class="flex min-w-0 flex-1 flex-col bg-white">
        <template v-if="draft && selectedScript">
          <header class="flex flex-wrap items-center gap-3 border-b border-slate-200 px-5 py-3.5">
            ...
            <span class="ml-auto text-xs text-slate-400">更新于 {{ new Date(selectedScript.updatedAt).toLocaleString() }}</span>
            <el-button :rows="15" type="primary" class="!h-9 !rounded-lg !font-semibold" :loading="isSaving" @click="saveScript">保存</el-button>
          </header>
          <div class="min-h-0 flex-1 bg-slate-50/40 p-4 sm:p-5">
            <el-input v-model="draft.content" class="h-full font-mono" type="textarea" resize="none" placeholder="请输入 Pine Script 源码" />
          </div>
        </template>
        <div v-else class="flex flex-1 flex-col items-center justify-center text-sm text-slate-400">
          <span class="mb-3 flex size-12 items-center justify-center rounded-xl bg-blue-50 text-xl text-blue-500">⌘</span>
          请选择或新建一个脚本
        </div>
      </section>
    </div>
